...
class MultiTaskDataset(Dataset):
    def __init__(self, data_root: str, transforms_seg: Optional[A.Compose] = None, transforms_class: Optional[A.Compose] = None):
        super().__init__()
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Apply augmentations
        if task_name == 'segmentation' or task_name == 'detection':
            augmented = self.transforms_seg(image=image, mask=mask, bboxes=bboxes, class_labels=class_labels)
            image = augmented['image']
        else:
            augmented = self.transforms_class(image=image, mask=mask, bboxes=bboxes, class_labels=class_labels)
            image = augmented['image']            
        ...
        if task_name == 'segmentation':
            ...
            if augmented['bboxes']:
                ...
                w_box = x2 - x1
                h_box = y2 - y1
                label = np.array([cx, cy, w_box, h_box], dtype=np.float32)
            else:
