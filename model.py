...
class Model:
    ...
    def _process_detection(self, pred, task_id, image_path, original_size):
        ...
        """
        if pred.dim() == 3:
            pred = pred.unsqueeze(0)  # 变成 (1, C, H, W)
        ...
        pred_normalized = pred.view(B, 4, -1).mean(dim=2)  # (B, 4) in [0,1]
        ...
        bbox_norm = pred_boxes_xyxy[0]  # shape: (4,)
        if isinstance(bbox_norm, torch.Tensor):
            bbox_norm = bbox_norm.cpu().numpy()  # now shape (4,), dtype float
    
        bbox_norm_list = bbox_norm.tolist()  # [x1, y1, x2, y2]       
